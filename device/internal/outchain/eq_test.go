...
// Flat is a real bypass only when the default-on subsonic filter is explicitly
// disabled. The controller protects the driver by default, so zero user bands
// alone are not a passthrough.
func TestFlatIsPassthrough(t *testing.T) {
	eq := NewEQWithSettings(48000, make([]float64, NumBands), false, false,
...
// The default subsonic filter precedes the eight user bands. Loudness adds its
// presence and warmth pair rather than modifying those bands.
func TestLoudnessAddsASection(t *testing.T) {
	...
	if got := eq.Sections(); got != NumBands+1 {
		t.Errorf("sections = %d, want %d", got, NumBands+1)
	}
	eq.SetBands(bands, true)
	if got := eq.Sections(); got != NumBands+3 {
		t.Errorf("with loudness: sections = %d, want %d", got, NumBands+3)
	}
}

func TestCurrentEQSettings(t *testing.T) {
	bands := make([]float64, NumBands)
	eq := NewEQ(48000, bands, false)
	if eq.Flat() || eq.Sections() != NumBands+1 {
		t.Fatalf("default EQ = flat:%v sections:%d, want subsonic plus eight bands",
			eq.Flat(), eq.Sections())
	}

	// The configurable frequencies are clamped by the reference, while an
	// explicit disabled subsonic filter restores the true flat path.
	eq.SetSettings(bands, false, false, -1, 999)
	if !eq.Flat() || eq.Sections() != 0 {
		t.Fatalf("subsonic-off zero-band EQ = flat:%v sections:%d, want passthrough",
			eq.Flat(), eq.Sections())
	}
	eq.SetSettings(bands, false, true, -1, 999)
	if eq.bassShelfHz != minBassShelfHz || eq.subsonicHz != maxSubsonicHz {
		t.Errorf("clamped frequencies = shelf %.1f subsonic %.1f, want %.1f %.1f",
			eq.bassShelfHz, eq.subsonicHz, minBassShelfHz, maxSubsonicHz)
	}
}
