...
// Loudness is the reference's fixed presence and warmth pair, appended after
// the eight user bands.
const (
	presenceFreq   = 2500.0
	presenceGainDB = 4.0
	presenceQ      = 0.8
	...
	DefaultBassShelfHz = 125.0
	DefaultSubsonicHz  = 85.0
	minBassShelfHz     = 40.0
	maxBassShelfHz     = 300.0
	minSubsonicHz      = 20.0
	maxSubsonicHz      = 120.0
)
...
func highpassCoeffs(fc, fs float64) (b0, b1, b2, a1, a2 float64) {
	w0 := 2 * math.Pi * fc / fs
	cw := math.Cos(w0)
...
// EQ is the default-on subsonic filter, eight-band equaliser, and optional
// loudness pair.
//
...
type EQ struct {
	...
	sections                []biquad
	subsonic                bool
	bassShelfHz, subsonicHz float64
...
// NewEQ builds the chain for the given bands with the controller's default-on
// 85Hz subsonic protection. Use NewEQWithSettings for the full current
// controller configuration.
func NewEQ(sampleRate int, bands []float64, loudness bool) *EQ {
	return NewEQWithSettings(sampleRate, bands, loudness, true,
		DefaultBassShelfHz, DefaultSubsonicHz)

}

// NewEQWithSettings matches StreamingEQ's current frequency-shaping inputs.
func NewEQWithSettings(sampleRate int, bands []float64, loudness, subsonic bool,
	bassShelfHz, subsonicHz float64) *EQ {
	e := &EQ{fs: float64(sampleRate)}
	e.SetSettings(bands, loudness, subsonic, bassShelfHz, subsonicHz)
	return e
}

func clampHz(v, lo, hi float64) float64 { return math.Min(hi, math.Max(lo, v)) }
...
// loudness changes the count 9↔11 with default subsonic, so it is a state reset — which reads like an
...
func (e *EQ) SetBands(bands []float64, loudness bool) {
	e.SetSettings(bands, loudness, e.subsonic, e.bassShelfHz, e.subsonicHz)
}

// SetSettings updates the complete current controller EQ configuration while
// retaining section state whenever the section count permits it.
func (e *EQ) SetSettings(bands []float64, loudness, subsonic bool,
	bassShelfHz, subsonicHz float64) {
	b := normaliseBands(bands)
	if bassShelfHz == 0 {
		bassShelfHz = DefaultBassShelfHz
	}
	if subsonicHz == 0 {
		subsonicHz = DefaultSubsonicHz
	}
	bassShelfHz = clampHz(bassShelfHz, minBassShelfHz, maxBassShelfHz)
	subsonicHz = clampHz(subsonicHz, minSubsonicHz, maxSubsonicHz)
	e.subsonic, e.bassShelfHz, e.subsonicHz = subsonic, bassShelfHz, subsonicHz

	if !loudness && !subsonic && allZero(b) {
		e.flat = true
		e.sections = nil
		return
	}
	...
	offset := 0
	if subsonic {
		b0, b1, b2, a1, a2 := highpassCoeffs(subsonicHz, e.fs)
		e.sections[0].setCoeffs(b0, b1, b2, a1, a2)
		offset = 1
	}
	for i := 0; i < NumBands; i++ {
		...
		switch i {
		case 0:
			b0, b1, b2, a1, a2 = loShelfCoeffs(bassShelfHz, b[i], e.fs)
		case NumBands - 1:
			b0, b1, b2, a1, a2 = hiShelfCoeffs(EQFrequencies[i], b[i], e.fs)
		default:
			b0, b1, b2, a1, a2 = peakCoeffs(EQFrequencies[i], b[i], peakQ, e.fs)
		}
		e.sections[offset+i].setCoeffs(b0, b1, b2, a1, a2)
	}
	if loudness {
		b0, b1, b2, a1, a2 := peakCoeffs(presenceFreq, presenceGainDB, presenceQ, e.fs)
		e.sections[offset+NumBands].setCoeffs(b0, b1, b2, a1, a2)
		b0, b1, b2, a1, a2 = loShelfCoeffs(warmthFreq, warmthGainDB, e.fs)
		e.sections[offset+NumBands+1].setCoeffs(b0, b1, b2, a1, a2)
	}
}
