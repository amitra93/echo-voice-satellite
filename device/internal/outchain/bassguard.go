...
const (
	fullScale = 32768.0
	// A power of two, so multiplying by the reciprocal is exactly the
	// division it replaces rather than merely close to it.
...
type bandGain struct {
	...
	// The linear working set. The law is DEFINED in dB, but evaluating it
	// there costs a log per sample to enter and an exp per sample to leave,
	// and on this ARM core those were a large part of the 16-22ms the output
	// chain took out of a 42.7ms ALSA period.
	//
	// Release, floor and clamp are all monotonic under 10^(x/20), so they
	// have exact linear equivalents. The static curve does not — a ratio is
	// genuinely logarithmic — but it reduces to (level/threshold)^-expo, so a
	// sample UNDER the threshold needs no transcendental at all, and one over
	// it needs one rather than two.
	threshLin    float64
	invThreshLin float64
	floorLin     float64
	slewLin      float64
	expo         float64 // 1 - 1/ratio

	// gainLin is carried across chunks, LINEAR and <= 1. Its zero value is a
	// hard mute, so it must be initialised to unity — unlike the dB form it
	// replaces, whose zero value happened to be correct.
	gainLin        float64
...
func newBandGain(ratio, thresholdDB, releaseMS, floorDB, fs float64) *bandGain {
	...
	g.threshLin = math.Exp(g.thresholdDB * dbToLinear)
	g.invThreshLin = 1.0 / g.threshLin
	g.slewLin = math.Exp(g.slew * dbToLinear)
	g.expo = 1.0 - 1.0/g.ratio
	g.setFloorDB(floorDB)
	return g
}

// setFloorDB keeps the dB setting and its linear twin in step. The floor is
// the one parameter the dashboard changes at runtime, so it cannot be derived
// once at construction.
func (g *bandGain) setFloorDB(db float64) {
	g.floorDB = math.Min(0.0, db)
	g.floorLin = math.Exp(g.floorDB * dbToLinear)
}
...
func (g *bandGain) gains(x, out []float64) {
	...
	least := 1.0
	for i, v := range x {
		level := math.Abs(v) * invFullScale
		if level < epsLevel {
			level = epsLevel
		}

		// Static curve: above the threshold, keep 1/ratio of the excess.
		// -over*(1-1/ratio) dB is (level/threshold)^-expo as a linear gain,
		// so a sample under the threshold is unity with no arithmetic at all.
		target := 1.0
		if level > g.threshLin {
			target = math.Exp(-g.expo * math.Log(level*g.invThreshLin))
			if target < g.floorLin {
				target = g.floorLin
			}
		}
	...
	g.gainLin = prev
	if least < 1.0 {
		// One log per block, where the dB form needed one per sample.
		if worst := -20.0 * math.Log10(least); worst > g.maxReductionDB {
