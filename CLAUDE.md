...
**Release channel.** Home Assistant installs the stable `controller/` add-on,
which uses the same published image family as the standalone deployment.
Controller releases use `controller-v*` tags; the release workflow publishes
the versioned image and updates `:latest` for the stable deployment. The
add-on's `version:` must match the image tag being released.
...
turn engine talks to HA. STT is now *also* HACS-owned: `hacs/custom_components/echo_voice_satellite/stt.py` `GeminiTranscribeEntity` streams the same 16 kHz mono `MIC_PCM` via `google-genai` `Live` (`gemini-3.5-transcribe-live`, `LiveConnectConfig` + `AudioTranscriptionConfig` carrying `language_codes`/`mode`/`custom_vocabulary` from the `Configure` options flow, `response_modalities=["TEXT"]`, `Automatic` server VAD via `requires_external_vad=False`) — one Live session per turn provides both interim `interim_input_transcription` (forwarded as `POST /api/turns/{id}/transcript {is_final:false}` via `CorrelatedMicStream` + `ContextVar` to survive `process_enhance_audio` wrapping, `is_final` gated `transcript_mono`/`stt_latency_ms` on the controller) and final `input_transcription` (`SpeechResult` → `STT_END` → `is_final:true`). No HA `VoiceCommandSegmenter`, no `STT_VAD_*` events, no per-device STT entity — one shared entity per config entry, opt-in per pipeline (`STT engine` → `Gemini Transcribe`). Requires `google-genai>=2.22.0` (older `1.59.0` falls back to empty `AudioTranscriptionConfig`). See `hacs-stt-plan.md` (Implemented) and `hacs/README.md` `STT via Gemini`. TTS is received as HA generates it — the HACS
...
Production audio is Amazon AFE only. The system-UID helper opens paired OpenSL
`VOICE_RECOGNITION` capture and OpenSL playback; Amazon's HAL owns microphone
...
into a second buffer on the device, and the renderer mixes them before OpenSL
playback.
...
4. **Speaker** — the wire carries **mono** 48kHz; `_fetch_tts_audio` decodes at the wire rate (the satellite declares `supported_formats` 48k/mono/FLAC so HA transcodes at source when it can; ffmpeg resamples otherwise — no numpy resample step anymore). The AFE/OpenSL output route presents it to the hardware. Device buffers ~5.5s (`audioChanDepth`) and holds playback until ~1s is queued or EOS arrives (`primePeriods`) — WiFi-stall protection for marginal links
...
| `em_turn_engine.py` | The voice-turn state machine — wired into `em_api.py`, not a standalone server. Owns `trigger_voice_turn`/`cancel_voice_turn`, `_stream_mic_audio`, `_persist_turn`, and the per-turn audio-WS registry (`turn_id → ws`). Replaces `em_esphome.py` (deleted, Phase 4 cutover — see `docs/design/full-duplex-plan.md`). `transcript` action gates `transcript_mono`/`stt_latency_ms` on `is_final` and logs partials at `DEBUG text=` (dropped by `em_support`) |
...
| `hacs/custom_components/echo_voice_satellite/stt.py` | HACS-owned streaming STT: `GeminiTranscribeEntity` (`gemini-3.5-transcribe-live` `Live` + `AudioTranscriptionConfig`, `CorrelatedMicStream` + `ContextVar` for partials, `audio_processing requires_external_vad=False`, `>=2.22.0` with `1.59.0` fallback). See `hacs-stt-plan.md` |
...
The OpenSL helper and runtime bindings use the compiler Docker image's ARM
cross-toolchain. If adding cgo dependencies, they must compile cleanly with the
`echomuse-compiler` image against the FireOS 5 sysroot.
